-- 007 · 固定项支持收入型(房租等定期进账): recurring.flow = expense|income(转账仍用 peer_account_id)
alter table recurring add column if not exists flow text not null default 'expense'
  ...
  loop
    ...
    if r.period = 'monthly' then
      ...
      loop
        ...
        elsif r.flow = 'income' then
          perform record_tx(r.account_id, 'income', r.amount, r.category,
