...
import { store, recordTx } from '../lib/store'
import { EXPENSE_CATS, INCOME_CATS, todayStr, fmtMoney } from '../lib/fmt'
...
const cats = computed(() => (type.value === 'income' ? INCOME_CATS : EXPENSE_CATS))
...
<template>
  <div class="max-w-md mx-auto px-4 pt-6">
    ...
    <div v-if="!accounts.length" class="card p-6 text-center">
    ...
    <template v-else>
      ...
      <div class="flex gap-2 mb-4">
        <button v-for="t in [['expense', '支出'], ['income', '收入'], ['transfer', '转账']]" :key="t[0]"
          class="flex-1 py-2.5 rounded-xl text-sm font-medium border"
          :style="type === t[0]
            ? `background: ${t[0] === 'income' ? 'var(--c-in)' : t[0] === 'expense' ? 'var(--c-out)' : 'var(--c-net)'}; color:#fff; border-color: transparent`
      ...
      <div class="card p-4 mb-3">
        ...
        <div class="text-xs mt-1" style="color: var(--ink-3)">{{ account?.currency || '' }}</div>
      </div>
      ...
      <div v-if="type !== 'transfer'" class="card p-4 mb-3">
        <div class="text-xs mb-2" style="color: var(--ink-3)">分类</div>
      ...
      <div class="card p-4 mb-4 space-y-3">
        <input v-model="note" placeholder="备注(可选)" class="w-full bg-transparent outline-none" />
      ...
      <button :disabled="busy" class="w-full py-3.5 rounded-xl font-semibold text-white disabled:opacity-50"
        style="background: var(--c-net)" @click="submit">
        {{ busy ? '正在入账…' : '记入账本' }}
      </button>
